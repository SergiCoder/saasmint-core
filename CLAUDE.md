...
- **Seeding**: `seed_catalog` (idempotent, USD-only) → `sync_localized_prices` (recomputes `LocalizedPrice` rows from the FX feed) → `sync_stripe_catalog` (replaces placeholder `stripe_price_id`s with real ones, idempotent via `lookup_key`; for non-USD `BILLING_CURRENCIES` it stamps `LocalizedPrice.stripe_price_id`). All three run from `infra/entrypoint.sh` after `migrate` on every deploy in this order — `sync_localized_prices` must precede `sync_stripe_catalog` because the latter reads `LocalizedPrice.amount_minor` when minting non-USD Stripe Prices.
...
**What never changes**: `PlanPrice.amount` / `ProductPrice.amount` are always USD cents — the source of truth. `display_amount` is FE-only. Stripe charges the customer in their resolved billing currency (USD for display-only currencies; non-USD for currencies in `BILLING_CURRENCIES`). The Stripe Price used at checkout always matches the currency Stripe will charge, so display drift can never cause a wrong-currency charge.
