...
- `Product` / `ProductPrice` are one-time purchases (credit packs / Boost), separate from subscription plans. Purchases go through `POST /api/v1/billing/product-checkout-sessions/` (Stripe Checkout `mode=payment`) and accept the same `?context=personal|team` selector as subscription mutations (rule 5a/5b — default: `team` for `ORG_MEMBER` callers, `personal` otherwise). `?context=personal` is universally allowed (anyone can buy credits for themselves, including ORG_MEMBER admins/members who can't spend org funds); `?context=team` requires `role=OWNER` on the active org and 403s for admins/members. PERSONAL callers cannot pick `team` (400). On `checkout.session.completed` the webhook routes to `_on_product_checkout_completed` which grants credits via `CreditTransaction` + `CreditBalance`.
- Credit ledger: `CreditBalance` (denormalised per-user-or-org current balance, XOR `user`/`org`) + `CreditTransaction` (immutable audit log, unique on `stripe_session_id` for webhook-replay idempotency). `GET /api/v1/billing/credits/me/` returns a list envelope `{balances: [{balance, scope}, ...]}`. PERSONAL callers get a single `scope=user` entry; ORG_MEMBER callers get the `scope=org` entry (readable by any active member) plus a `scope=user` entry iff they have a leftover personal balance from a pre-rule-16-upgrade purchase (omitted when zero).
- Subscription mutations (`PATCH` / `DELETE /api/v1/billing/subscriptions/me/`) accept a `?context=personal|team` query param to pick which sub to mutate when the caller holds both concurrently (rule 5a/5b). Default: `team` for `ORG_MEMBER` callers, `personal` otherwise. The `is_billing=True` gate only applies to `?context=team` — any user may mutate their own personal sub. On `cancel_at_period_end` flips, `send_subscription_cancel_notice_task` emails every `is_billing=True` member (team) or just the owner (personal) so a rogue billing contact's action is visible.
...
- Personal→team upgrade (rule 16): a PERSONAL user can POST a team-context checkout; the 409 guard gates on "user already owns an org" (rule 8), not on `account_type`. Rule 8 is DB-enforced via a partial unique index on `OrgMember(user) WHERE role='owner'` (`uniq_org_owner_per_user`, migration `apps/orgs/migrations/0011_uniq_org_owner_per_user.py`); the view-layer `.exists()` check stays as a fast-path UX guard, while the constraint is the authoritative enforcer for the TOCTOU race where two parallel team checkouts both pass the application guard before either webhook fires. `_create_org_with_owner` flips `account_type` from PERSONAL to ORG_MEMBER atomically with org creation (one-way; never demotes). The `keep_personal_subscription` field on `CheckoutRequestSerializer` (default `false`) controls whether the user's existing personal sub is scheduled to cancel at period end on webhook completion; set `true` to leave personal running concurrently with the new team sub (rule 5b).
